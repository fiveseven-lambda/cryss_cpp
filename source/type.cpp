...
namespace type {
    ...
    const Tuple &Function::get_arguments_type() const { return arguments_type; }
    const Type &Function::get_return_type() const { return return_type; }
    std::pair<const Tuple &, const Type &> Function::into_pair() const { return {arguments_type, return_type}; }
    ...
    std::size_t FunctionHash::operator()(const std::pair<const Tuple &, const Type &> & pair) const noexcept {
        std::size_t seed = TupleHash()(pair.first.get_elements_type());
    ...
    bool FunctionEq::operator()(const std::pair<const Tuple &, const Type &> &left, const std::pair<const Tuple &, const Type &> &right) const noexcept {
        return TupleEq()(left.first.get_elements_type(), right.first.get_elements_type()) && (&left.second == &right.second);
    }
    bool FunctionEq::operator()(const std::pair<const Tuple &, const Type &> &left, const std::unique_ptr<Function> &right) const noexcept { return (*this)(left, right->into_pair()); }
    ...
    const Function &TypeContext::function(const Tuple &arguments_type, const Type &return_type){
        auto it = functions.find(std::pair<const Tuple &, const Type &>(arguments_type, return_type));
        if(it == functions.end()) it = functions.insert(std::make_unique<Function>(arguments_type, return_type)).first;
        return **it;
    }
